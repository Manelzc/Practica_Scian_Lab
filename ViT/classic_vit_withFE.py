...
class ViTWithFE(nn.Sequential):
    def __init__(self,     
        ...
        super().__init__(
            CombineFEwithEmbed(
                embedder= PatchEmbedding(in_channels, patch_size, emb_size, img_size),
                feature_extractor= feature_extractor,
                n_patches=img_size//patch_size,
                fe_size=fe_size
            ),
            TransformerEncoder(depth, emb_size=emb_size+fe_size//(img_size//patch_size)**2-1, **kwargs),
            ClassificationHead(emb_size+fe_size//(img_size//patch_size)**2-1, n_classes)
        )
