...
from patch_embedding import PatchEmbedding, ImagePatcher
...
class ConvolutionalStem(nn.Sequential):
    def __init__(self, in_channels:int=3, out_channels:int=12, n_patches:int=13, dilation:int=1):
        super().__init__(
            ...
            Rearrange('(b w h) c s1 s2 -> b (w h) (c s1 s2)', w=n_patches, h=n_patches)
        )
...
class Embedder(nn.Module):
    def __init__(self, in_channels: int = 3, patch_size: int = 16, emb_size: int = 768, n_patches: int = 16):
        ...
        self.positions = nn.Parameter(torch.randn((n_patches) **2 + 1, emb_size))
...
class MultiscaleViT(nn.Sequential):
    def __init__(self,     
        ...
        super().__init__(
            ImagePatcher(in_channels, patch_size, img_size, returnGrouped=False),
            #ParallelScale(
                #Printer(),
                ConvolutionalStem(in_channels, out_channels, n_patches=img_size//patch_size),
                Embedder(out_channels, outsize, emb_size, n_patches=img_size//patch_size),
